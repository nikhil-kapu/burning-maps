...
export function VerifyEmailScreen({ route }: Props) {
  ...
  const verify = async () => {
    ...
    try {
      throwClerkError((await signUp.verifications.verifyEmailCode({ code })).error, "That code did not work.");
      if (signUp.status !== "complete") throw new Error("Your account still needs additional information.");
      throwClerkError((await signUp.finalize()).error, "We could not finish creating your account.");
    }
    catch (value) { setError(clerkErrorMessage(value, "That code did not work.")); }
    finally { setLoading(false); }
  };
  const resend = async () => {
    if (seconds > 0) return;
    try {
      throwClerkError((await signUp.verifications.sendEmailCode()).error, "We could not send another code.");
  ...
  return (
    <Screen style={styles.screen}>
      <BrandMark />
      <View style={styles.mailIcon}><Text style={styles.mailEmoji}>✉</Text></View>
      <Text style={[typography.hero, styles.center]}>Check your email.</Text>
      <Text style={[typography.body, styles.center, styles.copy]}>We sent a six-digit code to <Text style={styles.strong}>{route.params.email}</Text>.</Text>
      ...
      <AppButton label="Verify and continue" loading={loading || fetchStatus === "fetching"} onPress={() => void verify()} />
...
const styles = StyleSheet.create({
  ...
  mailIcon: { width: 72, height: 72, borderRadius: 24, backgroundColor: colors.peach, alignItems: "center", justifyContent: "center", alignSelf: "center", marginTop: 52, marginBottom: 22, transform: [{ rotate: "-5deg" }] },
  mailEmoji: { fontSize: 32 },
